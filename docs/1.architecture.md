...
│
├── docs/                       # ─── 【アーキテクチャ・ドキュメンテーション】
│   ├── adr/                    # ─── Architecture Decision Records (建築的決定記録)
│   │   ├── 000-README.md       # ADR（意思決定記録）の概要と、記録フォーマットのガイドライン
│   │   ├── 001-separation-of-llm.md # [決定記録] LLM依存を排除し、Decision Layerを疎結合化した経緯
│   │   ├── 002-state-isolation.md    # [決定記録] GameStateをロジックを持たない純粋集約器に隔離した経緯
│   │   └── 003-real-time-simulation.md # [決定記録] 物理定数を分離し、Simulationをピュアに駆動させた経緯
│   │
│   ├── 0.design_notes.md       # 混沌からの脱却、初期の思考プロセスとリファクタリングの動機
│   ├── 1.architecture.md       # ← 最終設計：クリーンアーキテクチャの定義と単方向依存の保証
│   ├── 2.flow.md               # GemmaからSimulationEngineへ至る「一本のデータフロー」の解説
│   ├── 3.api_spec.md           # Decision Layer（InferenceNormalizer）のバリデーション・丸め仕様
│   ├── 4.state.md              # 4大ドメインStateと、集約器としての GameState の責務定義
│   ├── 5.threat.md             # ThreatEngineが認知を純粋な物理（Physics）へ変換するマッピング数式
│   └── 6.architecture_evolution.md # プロトタイプから堅牢な本番設計へ至る「設計の変遷」の記録
│
├── README.md/                  # ─── 【README.md フォルダ】
│   ├── 0. Introduction.md      # 導入部分、プロジェクト全体の概要と背景
│   ├── 1. Live System.md       # 現在動いている稼働システムの仕様・構造
│   ├── 2. Architecture Evolution.md # 設計がどう進化してきたか、過去の変遷ログ
│   ├── 3. Final Architecture.md # クリーンアーキテクチャ図と設計思想
│   └── 4. Future Work.md       # 今後の展望やマルチLLM対応予定
│
└── my_game_project/            # ─── 【ソースコード本体】
    ├── models/                 # ─── ドメイン・データモデル層（Immutable）
    │   ├── inference.py        # NormalizedInference
    │   └── physics.py          # PhysicsParameters
    ├── services/               # ─── 外部インフラ・通信層
    │   └── gemma.py            # Gemma APIとの通信
    ├── api/                    # ─── Decision Layer（判定・防衛層）
    │   └── inference.py        # InferenceNormalizer
    ├── state/                  # ─── 状態管理・集約パッケージ
    │   ├── __init__.py         
    │   ├── game_state.py       # GameState（純粋な集約器）
    │   ├── world_state.py      
    │   ├── cognitive_state.py  
    │   ├── simulation_state.py 
    │   └── telemetry_state.py  
