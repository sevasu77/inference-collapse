...
├── README.md                   # プロジェクト全体の概要、導入方法、およびクイックスタート
│
├── docs/                       # ─── 【アーキテクチャ・ドキュメンテーション】
│   ├── 0. Introduction.md      # プロジェクトの背景、目的、および本再設計が解決する課題の総論
│   ├── 1. Live System.md       # 稼働中のシステム概要、外部API（Gemma）との連携基盤の仕様
│   ├── 2. Architecture Evolution.md # プロトタイプから出発し、依存性逆転（DIP）へ至る設計の変遷・進化録
│   ├── 3. Final Architecture.md # 最終確定したクリーンアーキテクチャの定義、および単方向データフローの全貌
│   └── 4. Future Work.md       # [Update: 6.からリネーム] 将来的な拡張性（GPT/Claude対応、新物理パラメータ、ThreatAssessment層の追加など）
│
└── my_game_project/            # ─── 【ソースコード本体】
    ├── models/                 # ─── 【ドメイン・データモデル層（Immutable）】
    │   ├── inference.py        # NormalizedInference（正規化された型安全な推論オブジェクト）
    │   └── physics.py          # PhysicsParameters（Gemmaを1ミリも知らない純粋な物理定数）
    │
    ├── services/               # ─── 【外部インフレ・通信層】
    │   └── gemma.py            # Gemma APIとの通信、および生JSON（dict）への一次デコードを担う
    │
    ├── api/                    # ─── 【Decision Layer（判定・防衛層）】
    │   └── inference.py        # InferenceNormalizer（生dictを検証し、防波堤としてモデルへ落とし込む）
    │
    ├── state/                  # ─── 【状態管理・集約パッケージ】
    │   ├── __init__.py         # パッケージの窓口（GameStateのみを外部に綺麗にエクスポート）
    │   ├── game_state.py       # GameState（4つのStateを束ねる、ロジックを持たない純粋な集約器）
    │   ├── world_state.py      # 物理世界のノードクリア状況やプレイヤーの進捗を保持
    │   ├── cognitive_state.py  # AI（LLM）が世界をどう解釈したかの認知ログ・信頼度スコアを保持
    │   ├── simulation_state.py # ゲームの実行ループに関する動的なメタデータ（残り時間など）を保持
    │   └── telemetry_state.py  # 監査システム用のリアルタイムログや、最終答え合わせ用データを保持
    │
    └── engine/                 # ─── 【純粋演算・ループ駆動層】
        ├── threat.py           # ThreatEngine（AI認知を燃料とし、物理パラメータを「生成」する翻訳装置）
        └── simulation.py       # SimulationEngine（供給された物理定数だけを盲信して世界を動かすループ）
```
